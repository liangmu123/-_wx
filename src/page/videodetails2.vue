...
<template>
    <div class="videodetails navigation_bottom">
        <liveckplayer v-if="type && type == 'live'" :details="details"></liveckplayer>
        <ckplayer v-else :details="details"></ckplayer>
        <div class="content" v-if="type != 'live'">
...
<script>
    ...
    import {get_article,camera_index} from '@/api/api'
    // 用户足迹
    ...
    export default {
        ...
        methods:{
            get_article(){
                ...
                params.id = that.id
                params.device = "wx"
                that.$axios.post(camera_index,params)
                .then(res=>{
