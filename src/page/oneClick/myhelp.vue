<!-- 一点通个人中心 -->
<template>
    <div class="myhelp navigation_bottom" v-if="list.length>0">
        <yd-infinitescroll :callback="loadList" ref="infinitescrollDemo">
            <yd-list theme="1" slot="list">
                <template v-for="(item,index) in list" v-if="list.length > 0">
                    <secondlist :key="index" :details="item" :need="need" :type="type"></secondlist>
                </template>
            </yd-list>
            <!-- 数据全部加载完毕显示 -->
